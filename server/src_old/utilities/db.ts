...
/**
 * Case-insensitive exact match on a model's `name` column. SQLite's LIKE is
 * ASCII case-insensitive when the pattern has no wildcards, so this doubles
 * as an exact (not substring) match.
 */
